...
//     return await jobsService.setActiveJob(job.id, job.name, job.data, JOB_STATUS.ACTIVE, {});
// });

// worker.on('completed', async (job: Job) => {
//     return await jobsService.setCompletedJob(job.id, job.name, job.data, JOB_STATUS.COMPLETED, {
...
//     return await jobsService.setFailedJob(job.id, job.name, job.data, JOB_STATUS.FAILED, {
//         error: err.message,
//     });
// });
